...
describe('POST /users/login', () => {
    ...
    it('should log in user and return auth token', (done) => {
        request(app)
            ...
            .expect(200)
            .expect((res) => {
                expect(res.headers['auth-x']).toExist();
            })
            .end((err, res) => {
                if(err) {
                    return done();
    ...
    it('should reject invalid login', (done) => {
        request(app)
            ...
            .send({
                email: users[1].email,
                password: users[1].password + 1 //invalid
            })
            .expect(400)
            .expect((res) => {
                expect(res.headers['auth-x']).toNotExist();
            })
            .end((err, res) => {
                if(err) {
                    return done();
...
//NOT WORKING - NEED TO INSTALL MOCHA TEST FRAMEWORK IN ITELLIJ to debug!!!
